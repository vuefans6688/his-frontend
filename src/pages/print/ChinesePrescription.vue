<template>
  <ul class="prescription" v-aotoClose>
    <li class="item onePage" v-for="(item, index) in presList" :key="index">
      ...
      <el-row>
        <el-col :span="24" class="title"><h2>中药 处方笺</h2></el-col>
      </el-row>
      <el-row>
      ...
      <div class="container">
        <h1 class="rp">Rp</h1>
        <p class="chinese_list">
          <el-row>
            <el-col :span="8" v-for="(item_c, key) in item.drug_list" :key="key">
              {{item_c.name}} <i v-show="item_c.use_note">({{item_c.use_note}})</i> {{item_c.dosage}}{{item_c.dose_unit}}  
            </el-col>
          </el-row>
        </p>
        <p class="usage">
          <span>总共{{item.total || item.drug_list[0].total}}剂</span>
          <span>{{item.frequency || item.drug_list[0].frequency }}</span>
          <span>{{item.zjyongfa || item.drug_list[0].zjyongfa }}</span>
          <span>{{item.taking_note || item.drug_list[0].taking_note}}</span>
        </p>
...
export default {
  ...
  mounted() {
    ...
    this.presList = this.$store.state.printMsg.chinaMedPre;
