<template>
  <div class="treatment" v-aotoClose>
    ...
    <el-row>
        <el-col :span="24" class="title"><h2>{{title}}申请单</h2></el-col>
    </el-row>
    <div class="info">
        <el-row>
    ...
    <el-row class="footer">
        <el-col class="right" :span="20"><span class="label">申请医师:  {{customer.doctor}}</span></el-col>
...
export default {
  ...
  mounted() {
    let treatment = [];
    switch (this.$route.query.checkId) {
      case '3' :
        this.title = '诊疗项目';
        treatment = this.$store.state.printMsg.checkItem.items;
        break;
      case '4' :
        this.title = '耗材';
        treatment = this.$store.state.printMsg.comsumables.items;
        break;
      case '6' :
        this.title = '其他项目';
        treatment = this.$store.state.printMsg.other.items;
        break;
      default:
        this.title = '诊疗项目';
        treatment = this.$store.state.printMsg.checkItem.items;
