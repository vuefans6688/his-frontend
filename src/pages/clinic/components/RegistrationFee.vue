<template>
  <div class="treament" v-loading.fullscreen.lock="fullscreenLoading" element-loading-text="加载中,请稍后" 
    ...
    <div class="new_section">
      <el-button size="small" type="primary" @click="modify_registration(-1)">新增挂号项</el-button>
    </div>
    <div class="treaments">
      <el-table
        :data="registrations"
    ...
    <div class="modify_section" v-show="modify_registration_show">
      <div class="container">
        ...
        <h1 class="title">{{this.oparation}}挂号项</h1>
        <div class="content">
          <el-form :model="registration" :rules="rules_registration" ref="registration" label-width="100px" size="small">
            ...
            <el-form-item label="名称:" prop="name">
              <template solt-scope='scope'>
                <el-autocomplete popper-class="my-autocomplete" v-model="registration.name" :disabled="canEditName" @blur="checkName" :fetch-suggestions="querySearch" placeholder="请选择名称" :trigger-on-focus="false" @select="handleSelect">  
                  <template slot-scope="props">
                    <div class="thinkName">
                      {{ props.item.name }}
                      <p>社保编号：<span class="code">{{ props.item.code }}</span></p>
                    </div>
                  </template>
                </el-autocomplete>
              </template>
    ...
    update_item() {
      ...
      httpServerNormal(modifyChargitem, {
        ...
        items: [
          ...
          // { name: 'unit', newVal: this.registration.unit },
