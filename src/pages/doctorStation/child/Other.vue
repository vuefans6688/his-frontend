  ...
  <div class="preStyle">
    <div class="preTitle">
      <p>
        ...
        <span @click="usePlates('其它收费项目')">
          <i class="el-icon-document"></i> 调用模板
        </span>
      </p>
    </div>
    <div class="checkStyle">
      <!-- <Check Title='项目名称' :checkMsg="patientMsg.other" checkId="3" :patientMsg="patientMsg"></Check> -->
      <el-form class="demo-form-inline prescriptionInputs" align='center' size='small' ref="other" :model="patientMsg.other">
        ...
        <el-form-item v-for="(item, index) in patientMsg.other.items" :key='index'>
          ...
          <el-col :span="6">
            ...
            <el-form-item :prop="'items.' + index + '.name'" :rules="{required: true, message: '收费项名称不能为空', trigger: 'blur'}">
                <el-select v-model="item.name" placeholder="请选择收费项名称" :disabled="patientMsg.other.otherStatus.isDisabled" v-if="item.status == 0">
          ...
          <el-col :span="4">
            <el-form-item class="dose" align="left" :prop="'items.' + index + '.total'" :rules="{required: true, message: '数量不能为空', trigger: 'blur' }">
                ...
                <el-input v-model="item.total" class="inlineBlock" @keyup.native='numberInp("total")' @blur="calculate(index)" :disabled="patientMsg.other.otherStatus.isDisabled" v-if="item.status == 0"></el-input>
        ...
        <el-form-item align='left'>
          <span class='addinputs' @click='addFormItem' v-show="!patientMsg.other.otherStatus.isDisabled">
        ...
        <el-form-item align='right'>
          <div class="doctorMsg">
            ...
            <el-button type="primary" @click="submitForm('other')" size='small' v-show='!patientMsg.other.otherStatus.isDisabled'>保 存</el-button>
    ...
    <div class="usePlate" v-show='usePlateWin'>
      <div class="container">
        ...
        <UsePlates :templateList='templateList' :templateType='7' @callball="templateCallBack"></UsePlates>
      </div>
    </div>
    <!--保存为模板窗  -->
    <SavaItemTemplate :items='patientMsg.other.items' :itemtype='7' v-if='savaPlateWin' @cancel='cancelSaveTemplate'></SavaItemTemplate>
  </div>
</template>
...
import { saveChargitem, getChargitemList } from '@apiNormal';
...
export default {
  ...
  methods: {
    ...
    calculate(index) {  // 计算金额
      ...
      this.patientMsg.other.items[index].total *
        this.patientMsg.other.items[index].price; // 计算当前价格;
      this.calculateAll();
    },
    calculateAll() {  // 计算总价
      let sum = 0;
    ...
    addFormItem() {
      ...
      const json = {
        ...
        chargitem_id: '',
        name: '',
        item_type: 6, // 0-未知，1挂号、2处方、3检查项、4耗材、5门诊购药、6其他"name": "抽血检查",
        total: '1',
        price: 0,
        total_price: 0,
    ...
    numberInp(str) { // 限制数字输入
      for (let i = 0; i < this.patientMsg.other.items.length; i++) {
        this.patientMsg.other.items[i][str] =
        numberLimit(this.patientMsg.other.items[i][str]);
      }
    },
  ...
  watch: {
    patientMsg() {
      if (this.patientMsg.otherPayList.length) {
        ...
        // this.patientMsg.other.items.map((val) => {
        //   val.total += '';
        //   return this.patientMsg.other.items;
        // });
      }
    },
  },
