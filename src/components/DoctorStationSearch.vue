      ...
      <el-collapse-item :name="index + 1" v-for="(item,index) in collapse" :key='index'>
        <template slot="title">
          <div class="collapseTitle">
              <!-- <el-checkbox @change="getTitle(item.titleName, item.checkbox)" v-model="item.checkbox" class="elCheckBox"></el-checkbox>   -->
            <!-- {{ item.titleName }} -->
             <el-tooltip class="item" effect="light" :content="item.titleName" placement="left">
              <span @click.stop="getTitle(item.titleName)" class="titleName">{{ item.titleName }}</span>
            </el-tooltip>
          </div>
        </template>
